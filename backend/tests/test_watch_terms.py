...
class TestCreateWatchTerm:
    ...
    def test_device_create_notify_term_requires_server_environment_verified_device(self, client, db_session):
        token = "a" * 64
        secret = "device-secret-value"
    ...
    def test_device_update_notify_term_requires_server_environment_verified_device(self, client, db_session):
        token = "a" * 64
        secret = "device-secret-value"
        owner_secret = hashlib.sha256(secret.encode()).hexdigest()
        term = WatchTerm(
